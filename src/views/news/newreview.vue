					...
					<div>
						<el-table ref="multipleTable" :data="listData" tooltip-effect="dark" style="width: 100%">
							...
							<el-table-column label="新闻名称" align="center">
								<template slot-scope="scope">{{ scope.row.cpbNews.name }}</template>
							</el-table-column>
							<el-table-column label="新闻内容" align="center">
								<template slot-scope="scope">{{ scope.row.cpbNews.content }}</template>
								</el-table-column>
								<el-table-column label="新闻时间" align="center">
									<template slot-scope="scope">{{ scope.row.cpbNews.date |formatDate }}</template>
									</el-table-column>
								<el-table-column label="查询" align="center">
									<template slot-scope="scope">
										<el-button size="mini" round type="success" @click="newsDetailInfo(scope.row.newsId)">详情信息</el-button>
									</template>
								</el-table-column>
						</el-table>
						<ToolBar>
	...
	export default {
		...
		methods: {
			...
			newsDetailInfo(id){
				...
				this.$router.push({path:'/newsReviewInfo',query:{id:id}})
			},
